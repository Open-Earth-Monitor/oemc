    ...
    <DropdownMenuTrigger
      className={cn({
        'flex h-full items-center justify-center space-x-4 border-l border-r border-secondary-900  text-center font-satoshi font-bold transition-colors hover:bg-secondary-900 data-[state=open]:bg-secondary-500 data-[state=open]:text-brand-500 lg:min-w-[180px]':
          !isMobile,
        'flex h-full w-full items-center justify-center py-4 text-center font-satoshi font-bold text-secondary-500 transition-colors first:border-t first:border-secondary-900 hover:bg-secondary-500 hover:text-brand-500':
          isMobile,
      })}
    ...
    >
      {navSubLinksCommunity.map(({ name, ...props }) => {
        return (
          <Link
            className={cn({
              'justify-left flex h-full items-center px-8 py-2 text-center font-satoshi font-bold text-secondary-500 transition-colors hover:bg-secondary-900 lg:min-w-[180px]':
                true,
            })}
            key={props.href}
            {...props}
