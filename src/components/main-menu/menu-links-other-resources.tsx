  ...
  return (
    <ul className="flex w-full flex-col gap-1 px-6 py-5 font-medium">
      {otherResources.map(({ name, ...props }) => {
        ...
        return (
          <li key={props.href} className="group/resource-link flex w-full cursor-pointer">
            <Link
            ...
            >
              {isActive && (
                <div className="h-2 w-2 rounded-full bg-black-500 group-hover/resource-link:bg-white-500" />
              )}
              <span>{name}</span>
            </Link>
          </li>
