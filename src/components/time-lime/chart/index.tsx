  ...
  return (
    <svg width={'100%'} height={'100%'} className="flex" fill="red">
      {filteredData.map((dt, index) => {
        ...
        return (
          <Group key={`bar-${dt.year}-${selectedYear}-${id}`}>
            <Bar
              x={x}
              y={yCenter}
              height={22}
            ...
            {isSelectedYear && x && yCenter - 18 && (
              <g>
                <Bar x={x} y={yCenter - 18} height={40} width={1} fill={'white'} />
                <HtmlLabel
                  x={x}
                  y={15}
                  horizontalAnchor={!isEdge ? 'middle' : index === 0 ? 'start' : 'end'}
                  verticalAnchor="middle"
                >
                  <div className="font-inter relative flex rounded-3xl border-none bg-brand-50 px-2 py-1 text-xs text-secondary-500">
                    {selectedYear}
                    {!isEdge && (
                      <div className="-transform absolute -bottom-1.5 left-1/2 h-0 w-0 -translate-x-1/2 transform border-x-8 border-t-[10px] border-x-transparent border-y-brand-50 border-b-brand-50" />
                    )}
                  </div>
                </HtmlLabel>
              </g>
