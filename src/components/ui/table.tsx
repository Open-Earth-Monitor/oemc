...
const Table = React.forwardRef<HTMLTableElement, React.HTMLAttributes<HTMLTableElement>>(
  ({ className, ...props }, ref) => (
    <div className="h-full max-h-[calc(100vh-18rem)] w-full overflow-auto">
      <table
        ref={ref}
        className={cn({ 'w-full caption-bottom': true, [className]: !!className })}
        {...props}
      />
    </div>
  )
...
const TableBody = React.forwardRef<
  ...
  React.HTMLAttributes<HTMLTableSectionElement>
>(({ className, ...props }, ref) => (
  <tbody ref={ref} className={cn('py-4', className)} {...props} />
));
...
>(({ className, ...props }, ref) => (
  <th
    ref={ref}
    className={cn(
      'py-5 text-left align-middle text-xs font-normal [&:has([role=checkbox])]:pr-0',
      className
    )}
    {...props}
  />
